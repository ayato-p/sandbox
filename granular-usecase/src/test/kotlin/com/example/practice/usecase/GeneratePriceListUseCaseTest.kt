...
import com.example.practice.usecase.port.*
...
class GeneratePriceListUseCaseTest {
    ...
    fun `商品グループと販売チャネルのあらゆる組み合わせから価格エントリが生成され逐次保存される`() {
        ...
        val productGroupHeadPort = FakeProductGroupHeadPort(groupHeads)
        val productPort = FakeProductPort(products)
        val salesChannelHeadPort = FakeSalesChannelHeadPort(channelHeads)
        val regionPort = FakeRegionPort(regions)

        val count = context(
            productGroupHeadPort::findAll,
            productPort::findByGroup,
            salesChannelHeadPort::findAll,
            regionPort::findByChannel,
            priceEntryPort::save
        ) {
            executeGeneratePriceList()
        }
